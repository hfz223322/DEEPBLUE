...
cv::Mat get_gaussian_kernel(int row, int col, cv::Size size, double sigma) {
    cv::Mat result = cv::Mat::zeros(size, CV_64F);
    for (int r = 0; r < result.rows; r++) {
        for (int c = 0; c < result.cols; c++) {
            result.at<double>(r, c) =
                (M_1_PI * 0.5 / (sigma * sigma)) *
                exp(-(pow(r - row, 2) + pow(c - col, 2)) /
                    (2 * sigma * sigma));
        }
    }
    return result / cv::sum(result)[0];
...
cv::Mat heaviside(cv::Mat map, double eps) {
    ...
    result = cv::Mat::ones(map.size(), CV_64F);
    for (int r = 0; r < result.rows; r++)
        for (int c = 0; c < result.cols; c++)
            result.at<double>(r, c) = heaviside(map.at<double>(r, c), eps);
    return result;
}
...
cv::Mat dirac(const HeightMap& sdf_map, double eps) {
    ...
    cv::Mat map = sdf_map.get_map();
    result = cv::Mat::ones(map.size(), map.type());
    for (int r = 0; r < result.rows; r++)
        for (int c = 0; c < result.cols; c++)
            result.at<double>(r, c) = dirac(map.at<double>(r, c), eps);
    return result;
}
...
cv::Mat compute_derivative_data_term(const HeightMap& sdf_map,
    ...
    cv::Mat dirac_mat = dirac(sdf_map, eps);
    cv::pow(original_image - center_foreground, 2.0f, e_foreground);
    cv::pow(original_image - center_background, 2.0f, e_background);
    result = dirac_mat.mul(weight_foreground * e_foreground - weight_background * e_background);
...
cv::Mat compute_derivative_length_term(const HeightMap& sdf_map, double eps) {
    ...
    cv::Mat dirac_mat = dirac(sdf_map, eps);
    cv::Mat div_delta_mat = compute_div_delta_map(sdf_map);
    result = dirac_mat.mul(div_delta_mat);
    return result;
}
...
cv::Mat compute_derivative_gradient_term(const HeightMap& sdf_map) {
    ...
    result = compute_laplacian_map(sdf_map) - compute_div_delta_map(sdf_map);
    return result;
}
...
double compute_data_term_energy(const HeightMap& height_map,
    ...
    cv::Mat data_term_energy_b, data_term_energy_f, heaviside_b, heaviside_f, e_b, e_f;
    heaviside_b = heaviside(height_map, eps);
    heaviside_f = complementary_heaviside(height_map, eps);
    cv::pow(original_image - center_background, 2.0f, e_b);
    cv::pow(original_image - center_foreground, 2.0f, e_f);
    result = weight_background * e_b.dot(heaviside_b) + weight_foreground * e_f.dot(heaviside_f);
    return result;
}

double compute_data_term_energy(cv::Mat height_map,
                                cv::Mat original_image,
                                double weight_foreground,
                                double weight_background,
                                double center_foreground,
...
double compute_length_term_energy(const HeightMap& height_map, double eps) {
    ...
    cv::Mat gradnorm = compute_mat_grad_magnitude(heaviside_map);
    result = cv::sum(gradnorm)[0];
    return result;
}
...
double compute_gradient_preserve_energy(const HeightMap& height_map) {
    ...
    result = height_map.get_gradient_magnitude_level_set();
    return result;
}
...
void visualize_lvl_set_segemenation(cv::Mat origin_img, const HeightMap& phi,
    ...
    /**std::cout << vis_sdf_draw.channels() << std::endl;**/
    ...
    /**std::cout << vis_seg_image.channels() << std::endl;**/
