...
HeightMap::HeightMap(int rows, int cols, cv::Point center, double radius)
    ...
    for (int r = 0; r < map_.rows; r++)
        ...
        {
            double distance;
            distance = std::pow(c - center.x, 2.0) + std::pow(r - center.y, 2.0);
            distance = std::sqrt(distance);
            map_.at<double>(r, c) = distance - radius;
        }
        std::cout << map_.channels() << std::endl;
}
...
double HeightMap::get_gradient_magnitude_level_set() const{
...
cv::Mat HeightMap::get_contour_points() const {
    ...
    for (int r = 0; r < map_.rows; r++)
        for (int c = 0; c < map_.cols; c++)
            if (abs(map_.at<double>(r, c)) <= 2)
                contour_vec.push_back(cv::Vec2d(c, r));
